...
function staff_validate_token($conn, $token) {
    ...
    if (!$isSuperadmin && legacy_date_is_set($row['desde'] ?? null) && $today < $row['desde']) return null;
    if (!$isSuperadmin && legacy_date_is_set($row['hasta'] ?? null) && $today > $row['hasta']) return null;
